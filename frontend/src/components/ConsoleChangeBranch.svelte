...
{#if typeof data === 'object' && data != null}
	{#if Object.keys(data).length != 0}
		<pre class="text-console m-0 text-cyan">{rightShiftLines('----------', shift)}</pre>
		{#each Object.entries(data) as [changeKey, changeValue]}
			<pre class="text-console m-0 text-cyan">{rightShiftLines(changeKey, shift)}<span
	...
	{/if}
{:else}
	<pre class="text-console m-0 text-green">{rightShiftLines((data + '').trim(), shift)}</pre>
{/if}
